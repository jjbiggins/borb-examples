...
class Code39(Barcode):
    ...
    def build(self):
        chars = [code39.EDGE]
        chars.extend(code39.MAP[char][1] for char in self.code)
...
class Code128(Barcode):
    ...
    def _calculate_checksum(self, encoded):
        cs = [encoded[0]]
        cs.extend(i * code_num for i, code_num in enumerate(encoded[1:], start=1))
        return sum(cs) % 103
    ...
    def build(self):
        ...
        code = "".join(code128.CODES[code_num] for code_num in encoded)
        code += code128.STOP
        code += "11"
        return [code]
