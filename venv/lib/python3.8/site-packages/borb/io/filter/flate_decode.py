...
class FlateDecode:
    ...
    def decode(
        ...
        current_row: List[int] = [0 for _ in range(bytes_per_row)]
        prior_row: List[int] = [0 for _ in range(bytes_per_row)]
        number_of_rows = len(bytes_after_zlib) // bytes_per_row

        # easy case
        bytes_after_predictor = [int(x) for x in bytes_after_zlib]
        if predictor == 2 and bits_per_component == 8:
        ...
        while pos + bytes_per_row <= len(bytes_after_zlib):
            ...
            current_row = list(bytes_after_zlib[pos : pos + bytes_per_row])
            pos += bytes_per_row
            ...
            # write current row
            bytes_after_predictor.extend(current_row[i] for i in range(len(current_row)))
            # Swap curr and prior
