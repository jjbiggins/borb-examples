...
def add_base_methods(object: typing.Any) -> typing.Any:
    ...
    def image_hash_method(self):
        ...
        hashcode = 1
        for p in pixels:
            if isinstance(p, (typing.List, typing.Tuple)):
...
class Function(Dictionary):
    ...
    def _get_sample_number(self, sample: typing.List[oDecimal]) -> typing.Optional[int]:
        ...
        return sum(F[i] * int(sample[i]) for i in range(len(sample)))

    def _get_sample(self, sample_number: int) -> typing.List[oDecimal]:
        n: int = len(self["Range"]) // 2
        ...
        return [
            Decimal(int(byte_str[i : i + bps], 2))
            for i in range(0, len(byte_str), bps)
        ]
    ...
    def evaluate(self, xs: typing.List[oDecimal]) -> typing.List[oDecimal]:
        ...
        if "FunctionType" in self and int(self["FunctionType"]) == 0:
            ...
            decode = self["Decode"] if "Decode" in self else range2
            # When a sampled function is called, each input value xi , for 0 £ i < m, shall be clipped to the domain:
            xs_prime = [
                min(max(xs[i], domain[2 * i]), domain[2 * i + 1]) for i in range(m)
            ]


            # That value shall be encoded:
            es = [Function._interpolate(
                    xs_prime[i],
                    domain[2 * i],
                    domain[2 * i + 1],
                    encode[2 * i],
            ...
            es_prime = [min(max(es[i], oDecimal(0)), size[i] - 1) for i in range(len(es))]
            ...
            rs_prime: typing.List[oDecimal] = [Function._interpolate(
        ...
        if "FunctionType" in self and int(self["FunctionType"]) == 2:
            ...
            return [c0[j] + xs[0] ** N * (c1[j] - c0[j]) for j in range(n)]
...
class String:
    ...
    def get_value_bytes(self):
        ...
        of this String
        """
        return list(self.get_content_bytes()) if self._encoding is None else None
